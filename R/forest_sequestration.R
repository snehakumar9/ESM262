forest_sequestration <- function(tree_density, avg_diameter, avg_height,
                                      carbon_fraction = 0.5, bm_conversion_factor = 0.1, growth_rate = 0.03) {
  # Documentation:
  ...
  # carbon_fraction: fraction of biomass that's carbon (on avg, 50% for diff tree parts)
  ...
  # > note: according to US Forest Service, it stands on avg at 3%. (0.03 in decimal)
  
  # Calculate averages from the individual tree observations in data:
  avg_diameter <- mean(tree_data$diameter, na.rm = TRUE)
  avg_height   <- mean(tree_data$height, na.rm = TRUE)
  
  # Combine tree dimensions into a single variable for clarity / modularity
