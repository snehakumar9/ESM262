...
# If raw tree data is provided by user, calculate the averages and density.
  ...
  # tree_dimensions <- diameter * height
...
```{r}
for(i in 1:nrow(tree_data)){tree_data$tree_dimensions = 
  tree_data$diameter[i]*tree_data$height[i]}

for(i in tree_data$treeID[i]) {
  tree_data$tree_biomass = forest_sequestration(diameter = tree_data$diameter,
